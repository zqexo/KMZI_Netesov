...
class HillCipher:
    def __init__(self, encryption_matrix, decryption_matrix, mod=26):
        ...
        self.decryption_matrix = np.array(decryption_matrix)  # Матрица для расшифрования
    ...
    def _text_to_vector(self, text):
        ...
        return [ord(char) - ord('A') for char in text.upper()]  # Конвертируем каждую букву в число

    def _vector_to_text(self, vector):
        """Преобразует числовой вектор обратно в текст"""
        return ''.join(chr(num + ord('A')) for num in vector)  # Конвертируем каждое число обратно в букву

    def _mod_matrix(self, matrix):
        """Вычисляет модуль матрицы по mod (для предотвращения отрицательных значений)"""
        return matrix % self.mod  # Берем остаток от деления каждого элемента матрицы на mod
    ...
    def encrypt(self, plaintext):
        ...
        block_size = self.encryption_matrix.shape[0]  # Размер блока равен количеству строк в матрице
        ...
        for i in range(0, len(vector), block_size):
            ...
            encrypted_block = self._matrix_mult(self.encryption_matrix, block)  # Умножаем матрицу на блок и берем mod
            encrypted_vector.extend(encrypted_block)  # Добавляем зашифрованный блок в общий вектор
    ...
    def decrypt(self, ciphertext):
        ...
        block_size = self.decryption_matrix.shape[0]  # Размер блока равен количеству строк в матрице

        # Расшифровываем каждый блок
        for i in range(0, len(vector), block_size):
            block = vector[i:i + block_size]  # Берем блок длины block_size
            decrypted_block = self._matrix_mult(self.decryption_matrix, block)  # Умножаем матрицу на блок и берем mod
            decrypted_vector.extend(decrypted_block)  # Добавляем расшифрованный блок в общий вектор
...
# Матрица M и её обратная M^-1
encryption_matrix = [
    [6, 24, 1],  # Строка 1 матрицы шифрования
    [13, 16, 10],  # Строка 2 матрицы шифрования
    [20, 17, 15]  # Строка 3 матрицы шифрования
]

decryption_matrix = [
    [8, 5, 10],  # Строка 1 матрицы расшифрования
    [21, 8, 21],  # Строка 2 матрицы расшифрования
    [21, 12, 8]  # Строка 3 матрицы расшифрования
]
