...
def caesar_cipher(text, shift):
    ...
    for char in text:  # Проходим по каждому символу в тексте
        if char.isalpha():  # Проверяем, является ли символ буквой
            ...
            )  # Определяем базу для больших и маленьких букв
            encrypted_text += chr(
                (ord(char) + shift - shift_base) % 26 + shift_base
            )  # Шифруем букву
        else:
...
def main():
    ...
    print(
        f"Секретные ключи: A = {secret_key_a}, B = {secret_key_b}"
    )  # Выводим секретные ключи
    ...
    print(
        f"Открытые ключи: A = {public_key_a}, B = {public_key_b}"
    )  # Выводим открытые ключи
    ...
    print(
        f"Общий секретный ключ: A = {shared_secret_a}, B = {shared_secret_b}"
    ...
    decrypted_message = caesar_cipher(
        encrypted_message, -shared_secret_a
    )  # Дешифруем сообщение
    print(
